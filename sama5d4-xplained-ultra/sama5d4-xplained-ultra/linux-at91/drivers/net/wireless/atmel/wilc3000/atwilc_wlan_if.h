...
//
// Module Name:  atwilc_wlan_if.h
//
//
//////////////////////////////////////////////////////////////////////////////


#ifndef ATWILC_WLAN_IF_H
#define ATWILC_WLAN_IF_H

/*bug 3887: [AP] Allow Management frames to be passed to the host*/
#define ATWILC_AP_EXTERNAL_MLME 
#define ATWILC_P2P
#define TCP_ENHANCEMENTS
//#define MEMORY_STATIC
//#define ATWILC_FULLY_HOSTING_AP
...
#define ATWILC_MAC_INDICATE_STATUS		0x1
#define ATWILC_MAC_STATUS_INIT	-1
#define ATWILC_MAC_STATUS_READY 0
#define ATWILC_MAC_STATUS_CONNECT 1

#define ATWILC_MAC_INDICATE_SCAN 		0x2
...
typedef struct {
	...
	int commit;
} atwilc_wlan_cfg_set_t;

typedef struct {
	int start;
	uint32_t id;
	int commit;
} atwilc_wlan_cfg_get_t;

typedef struct {
	uint32_t id;
	void *buffer;
	uint32_t buffer_size;
} atwilc_wlan_cfg_val_t;
#endif
...
typedef void (*atwilc_tx_complete_func_t)(void *, int);

#define ATWILC_TX_ERR_NO_BUF (-2)
...
typedef struct {
...
#endif	
	int (*wlan_stop)(void);
	int (*wlan_add_to_tx_que)(void *, uint8_t *, uint32_t, atwilc_tx_complete_func_t);
	...
	int(*wlan_add_mgmt_to_tx_que)(void *, uint8_t *, uint32_t, atwilc_tx_complete_func_t);

	#ifdef ATWILC_FULLY_HOSTING_AP
	int(*wlan_add_data_to_tx_que)(void *, uint8_t *, uint32_t, atwilc_tx_complete_func_t);
	#endif
...
#define INFINITE_SLEEP_TIME		(ATL_Uint32)0xFFFFFFFF
