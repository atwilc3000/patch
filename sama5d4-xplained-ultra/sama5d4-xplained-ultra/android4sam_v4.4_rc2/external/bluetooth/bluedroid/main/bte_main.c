...
static void preload_wait_timeout(union sigval arg)
{
	...
	if(is_bt_fw_downloading()&& preload_retry_cb.BTDownloadRetries<4)
	{
	 	APPL_TRACE_ERROR1("...preload_wait_timeout while BT Firmware downloading: retries %d (Start timer again)...",preload_retry_cb.BTDownloadRetries);
		preload_start_wait_timer();
		preload_retry_cb.BTDownloadRetries++;
	...
	{
		preload_retry_cb.BTDownloadRetries=0;
