...
//[[ ATMEL_WIFI
    private final String AP_STA_CONNECTED = "/data/misc/wifi/hostapd/connected";
    private boolean mWifiDisabledWhenIdle = false;
    private boolean mWifiApDisabledWhenIdle = false;
	private static final long IDLE_MS_AT_DISCONNECTION_WITH_AP = 1000 * 6;
	private static final long IDLE_MS_AT_CONNECTION_WITH_AP = 1000 * 10;
...
//[[ ATMEL_WIFI
    private boolean setWifiEnabled(boolean enabled) {
        if(true) Slog.d(TAG, "mWifiDisabledWhenIdle : Turn wifi" + enabled);
        long ident = Binder.clearCallingIdentity();
        try {
            if (!mSettingsStore.handleWifiToggled(enabled)) {
    ...
    private void setWifiApEnabled(WifiConfiguration wifiConfig, boolean enabled) {
        obtainMessage(CMD_SET_AP, enabled ? 1 : 0, 0, wifiConfig).sendToTarget();
    }
//]]
...
//[[ ATMEL_WIFI
					Slog.d(TAG, "CMD_SCREEN_ON 1");

                    //updateBatteryWorkSource();
					removeMessages(CMD_DEVICE_IDLE);
	                if(mWifiDisabledWhenIdle) {
						mWifiDisabledWhenIdle = false;
	                    setWifiEnabled(true);
						Slog.d(TAG, "CMD_SCREEN_ON 2");
	                }
                    else if(mWifiApDisabledWhenIdle) {
                        mWifiApDisabledWhenIdle = false;;
...
//[[ ATMEL_WIFI
                    ...
                    if(mPluggedType == 0) {
                        //ATMEL_WIFI_WIFI, delay
                        mIdleMillis = IDLE_MS_AT_CONNECTION_WITH_AP;
//]]
...
//[[ ATMEL_WIFI
                            //sendMessage(CMD_DEVICE_IDLE);
							Slog.d(TAG, "CMD_SCREEN_OFF 2");
                            mIdleMillis = IDLE_MS_AT_DISCONNECTION_WITH_AP;
                            sendMessageDelayed(CMD_DEVICE_IDLE, mIdleMillis); 
//]]
                        }
                    }
                    break;
...
//[[ ATMEL_WIFI
                    ...
                    if(wifiState != WIFI_STATE_DISABLED && wifiState != WIFI_STATE_DISABLING) {
                        if(setWifiEnabled(false)) mWifiDisabledWhenIdle = true;
                    }
                    else {
                        File connected = new File(AP_STA_CONNECTED);
                        if(!connected.exists()) {
							ConnectivityManager cm = (ConnectivityManager)mContext.getSystemService(Context.CONNECTIVITY_SERVICE);
                            String[] tetheredIfaces = cm.getTetheredIfaces();
                            if(tetheredIfaces.length > 0) {
								Slog.d(TAG, "TURN OFF SOFTAP");
								mWifiConfig = mWifiStateMachine.syncGetWifiApConfiguration();
								setWifiApEnabled(mWifiConfig, false);
								mWifiApDisabledWhenIdle = true;
							}
                        }
                        else {
                            ConnectivityManager cm = (ConnectivityManager)mContext.getSystemService(Context.CONNECTIVITY_SERVICE);
                            String[] tetheredIfaces = cm.getTetheredIfaces();
                            if(tetheredIfaces.length > 0) {
                                sendMessageDelayed(CMD_DEVICE_IDLE, IDLE_MS_AT_CONNECTION_WITH_AP);
                            }
                        }
                    }                                
//]]
...
*/
//[[ ATMEL_WIFI
                    if (mScreenOff && mPluggedType != 0 && pluggedType == 0) {
                        mIdleMillis = IDLE_MS_AT_CONNECTION_WITH_AP;
//]]
                        long triggerTime = System.currentTimeMillis() + mIdleMillis;
                        if (DBG) Slog.d(TAG, "set idle timer for " + mIdleMillis + "ms");
